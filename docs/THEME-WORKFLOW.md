...
**Never edit the published theme.** Work on the unpublished copy, verify it
in a preview, then push the same files to the published one.

The two themes swapped roles on 20 September 2026. A third-party theme
(`ap-neotech-home-03`, `192607977764`) was published by hand at 16:02 and the
site ran on its demo content for about half an hour; the owner then
republished **Site Builder Stack — Development** (`191811453220`), which is
therefore now the **published** theme despite its name. **Site Builder Stack —
Live** (`191797854500`) is the **unpublished** copy used for previews. The two
carry identical code (every push since 17 September went to both). The original
purchased theme, `one-speaker` (`191794807076`), is still retained unpublished.

Practical rule: push to `191797854500` first and preview with
`?preview_theme_id=191797854500`; then push to `191811453220` with
`SBS_ALLOW_LIVE=1`. Do not publish `ap-neotech-home-03`: it drops every custom
section, template, the analytics layer, the structured data and agents.md.
