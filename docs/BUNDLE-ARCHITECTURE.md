...
## The fifth, sixth and seventh products

The Shopify Automation & Admin API Toolkit shipped the same day at $39.99,
also outside the bundle. The same reasoning applies with one addition: it is
platform-specific. A bundle that includes it is a bundle for Shopify stores
only, which is a different product from "the lifecycle" and should be named
as such if it is ever made ("the Shopify Stack": Launch System + SEO toolkit
+ Conversion toolkit + Shopify toolkit, or the Operations System + Shopify
toolkit as an "operate a Shopify store" pair). The plumbing note above
applies — `BUNDLE_COMPONENTS` and `STACK_TRUTH` would need a second
definition. Nothing is recommended until there is purchase evidence.

The Website Migration & Replatforming System shipped on 14 September 2026 at
$29.99, also outside the bundle. It is a project product rather than a stage
product: it applies for the weeks a site moves and then stops applying, which
makes it a poor bundle component — most buyers of the lifecycle bundle are not
migrating, and most migrators need it once. The natural pairings are with the
SEO toolkit (the pre-migration audit) and the Operations system (the
post-launch rhythm), and the product page says so with plain links rather than
a second bundle. If a "move it" bundle is ever made, the same plumbing note
applies.
...
The Agency & Client Delivery System shipped on 14 September 2026 at $39.99,
outside the bundle. It is the one product about the business rather than
the website, so it is the natural spine of a future "agency bundle" — this
product plus whichever technical products the agency sells (Launch + SEO +
Conversion for a build-and-grow agency; Operations + Migration for a
maintenance-and-moves agency). Its own `reference/bundle-readiness.md`
documents the pairings; nothing in it depends on the others, and every
reference to them is marked "if owned". The existing Complete Site Builder
Stack is unchanged.
