...
Seven digital products plus a bundle of the first three, each with its own page
...
seven-product row, the bundle CTA beneath it, and the contextual links in the
guides.
...
- Claude Code Shopify Automation & Admin API Toolkit — $39.99 — automate the Shopify admin (added 13 September 2026; not in the bundle; Shopify stores only)
- Claude Code Website Migration & Replatforming System — $29.99 — move the site without losing what it has (added 14 September 2026; not in the bundle; the sixth lifecycle stage, "Migrate")
- Claude Code Agency & Client Delivery System — $39.99 — run the engagement around all of it for clients (added 14 September 2026; not in the bundle; the seventh stage, "Deliver"; the only product about the business rather than the website)
- Complete Site Builder Stack — $39.99 — the first three, as three downloads on one order

Four products ship code (the migration system carries 28 standard-library scripts; the agency system four). The operations system carries eleven standard-library
Python scripts alongside the Markdown; the Shopify toolkit carries thirty-two,
including an Admin GraphQL client, and its own mock-API test harness
(`scripts/test-shopify-toolkit.py`) so the suite proves every script — reads,
guarded writes, bulk operations — without touching a store. Its GraphQL
documents were validated against the live 2026-07 schema
(`product/.../scripts/validate_graphql.py`), which is also how the 2026-07
changes to collections, media and bulk-operation status were found. `validate-ops-system.py`
...
seven-product row explains which product suits which problem and a single line
