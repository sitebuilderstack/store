...
def motif_review_rows(d, x, y, acc):
    """Bulk SEO editing: rows of proposed metadata, each with a decision mark.

    The point of the article is the middle column, so the motif is a review
    table: approved, approved, unchanged, rejected. Not a spreadsheet — a
    spreadsheet would say "bulk"; this says "bulk, with a person in it"."""
    d.rounded_rectangle([x, y, x + 300, y + 340], 10, fill=SURFACE, outline=BORDER)
    d.text((x + 24, y + 22), "SEO REVIEW", font=font(MONO_B, 17), fill=acc)
    d.line([(x + 16, y + 56), (x + 284, y + 56)], fill=BORDER, width=2)
    states = ["ok", "ok", "same", "no"]
    for i, st in enumerate(states):
        ry = y + 76 + i * 64
        d.rounded_rectangle([x + 20, ry, x + 200, ry + 12], 4, fill=BORDER)
        d.rounded_rectangle([x + 20, ry + 22, x + 156, ry + 32], 4,
                            fill=MUTED if st in ("ok", "same") else BORDER)
        cx, cy = x + 242, ry + 14
        if st == "ok":
            d.line([(cx - 10, cy), (cx - 2, cy + 9), (cx + 12, cy - 10)],
                   fill=acc, width=4, joint="curve")
        elif st == "no":
            d.line([(cx - 9, cy - 9), (cx + 9, cy + 9)], fill=(244, 114, 182), width=4)
            d.line([(cx + 9, cy - 9), (cx - 9, cy + 9)], fill=(244, 114, 182), width=4)
        else:
            d.line([(cx - 10, cy), (cx + 10, cy)], fill=DIM, width=4)


def motif_page_blocks(d, x, y, acc):
    """Digital product page template: the page, as stacked blocks.

    A hero, a price and button, a preview panel, then the quieter blocks.
    The preview panel is drawn in the accent because that is the section the
    article argues most pages leave out."""
    d.rounded_rectangle([x, y, x + 300, y + 340], 10, fill=SURFACE, outline=BORDER)
    d.rounded_rectangle([x + 22, y + 24, x + 226, y + 42], 4, fill=MUTED)
    d.rounded_rectangle([x + 22, y + 52, x + 170, y + 64], 4, fill=BORDER)
    d.text((x + 22, y + 84), "$--.--", font=font(MONO_B, 24), fill=TEXT)
    d.rounded_rectangle([x + 22, y + 122, x + 160, y + 156], 8, fill=acc)
    d.rounded_rectangle([x + 22, y + 174, x + 278, y + 258], 8,
                        fill=SURFACE2, outline=acc, width=2)
    d.text((x + 36, y + 188), "output", font=font(MONO, 15), fill=acc)
    for i in range(3):
        d.rounded_rectangle([x + 36, y + 214 + i * 14, x + 36 + (200 - i * 42),
                             y + 222 + i * 14], 3, fill=BORDER)
    for i in range(3):
        d.rounded_rectangle([x + 22, y + 278 + i * 20, x + 22 + (236 - i * 30),
                             y + 288 + i * 20], 3, fill=BORDER)


def motif_scope_split(d, x, y, acc):
    """Scope of work: what is in, and what is explicitly out.

    Two columns. The right-hand one is the exclusions, drawn dimmer and
    struck through, because the article's argument is that the excluded
    column is the one that prevents the dispute."""
    d.rounded_rectangle([x, y, x + 300, y + 340], 10, fill=SURFACE, outline=BORDER)
    d.text((x + 22, y + 22), "IN", font=font(MONO_B, 16), fill=acc)
    d.text((x + 172, y + 22), "OUT", font=font(MONO_B, 16), fill=DIM)
    d.line([(x + 150, y + 18), (x + 150, y + 318)], fill=BORDER, width=2)
    for i in range(6):
        ry = y + 58 + i * 42
        d.rounded_rectangle([x + 22, ry, x + 22 + (108 - (i % 3) * 18), ry + 11],
                            3, fill=MUTED if i < 4 else BORDER)
    for i in range(5):
        ry = y + 58 + i * 42
        w = 96 - (i % 2) * 22
        d.rounded_rectangle([x + 172, ry, x + 172 + w, ry + 11], 3, fill=BORDER)
        d.line([(x + 168, ry + 6), (x + 176 + w, ry + 6)], fill=DIM, width=2)


def motif_extract(d, x, y, acc):
    """Answer engine optimization: one paragraph lifted out of a page.

    A page of muted lines with a single block pulled clear of it, drawn in the
    accent with quote marks. The argument of the article is that the unit is
    the extractable paragraph, not the page, so the motif shows exactly that
    and nothing else."""
    d.rounded_rectangle([x, y, x + 210, y + 340], 10, fill=SURFACE, outline=BORDER)
    for i in range(13):
        ly = y + 28 + i * 23
        if 4 <= i <= 6:
            continue
        w = 150 - (i % 4) * 22
        d.rounded_rectangle([x + 20, ly, x + 20 + w, ly + 9], 3, fill=BORDER)
    d.rounded_rectangle([x + 14, y + 118, x + 196, y + 186], 6,
                        fill=SURFACE2, outline=acc, width=2)
    for i in range(3):
        d.rounded_rectangle([x + 28, y + 132 + i * 18, x + 28 + (150 - i * 30),
                             y + 140 + i * 18], 3, fill=MUTED)
    # the lift: an arrow to a standalone quoted block
    d.line([(x + 200, y + 152), (x + 246, y + 152)], fill=acc, width=3)
    d.polygon([(x + 246, y + 145), (x + 258, y + 152), (x + 246, y + 159)], fill=acc)
    d.rounded_rectangle([x + 268, y + 104, x + 300 + 40, y + 200], 8,
                        fill=SURFACE, outline=acc, width=2)
    d.text((x + 284, y + 116), '\u201c', font=font(SANS_B, 46), fill=acc)
    for i in range(3):
        d.rounded_rectangle([x + 284, y + 158 + i * 14, x + 284 + (44 - i * 8),
                             y + 165 + i * 14], 3, fill=MUTED)
...
ARTICLES += [
    ...
    dict(slug="claude-code-website-maintenance",
         eyebrow="OPERATIONS CHECKLIST",
         title="Website Maintenance: A Safe Weekly Checklist",
         kicker="Inspect on a schedule, change nothing, then fix one thing at a time",
         accent=(94, 234, 212), motif=motif_gauge),
    dict(slug="website-migration-seo-checklist-claude-code",
         eyebrow="MIGRATION CHECKLIST",
         title="Website Migration SEO Checklist",
         kicker="Inventory, map, redirects that land where they should, and staging that says so",
         accent=(134, 239, 172), motif=motif_seo),
    dict(slug="claude-code-playwright-website-testing",
         eyebrow="TESTING TUTORIAL",
         title="Playwright Testing With Claude Code",
         kicker="Forms, links and regression checks — run, broken on purpose, fixed",
         accent=(138, 166, 255), motif=motif_ci),
]


ARTICLES += [
    dict(slug="bulk-edit-shopify-seo-claude-code",
         eyebrow="CATALOGUE WORKFLOW",
         title="Bulk Edit Shopify SEO With Claude Code",
         kicker="Export, review, approve, apply, verify",
         accent=(138, 166, 255), motif=motif_review_rows),
    dict(slug="shopify-digital-product-page-template",
         eyebrow="PAGE TEMPLATE",
         title="Shopify Digital Product Page Template",
         kicker="Twelve sections, and the one most pages leave out",
         accent=(94, 234, 212), motif=motif_page_blocks),
    dict(slug="website-scope-of-work-template-claude-code",
         eyebrow="CLIENT DELIVERY",
         title="Website Scope of Work Template",
         kicker="Deliverables, exclusions, and what counts as done",
         accent=(251, 191, 36), motif=motif_scope_split),
]

ARTICLES += [
    dict(slug="answer-engine-optimization-claude-code",
         eyebrow="ANSWER ENGINES",
         title="Answer Engine Optimization",
         kicker="The unit is the paragraph, not the page",
         accent=(167, 139, 250), motif=motif_extract),
]
