#!/usr/bin/env bash
#
# Build the distributable Agency & Client Delivery System archive.
#
#   ./scripts/build-agency-system.sh [version]
#
# Same shape as the other six product builds: validate, generate the
...
BUNDLE="$ROOT/product/Claude-Code-Agency-Client-Delivery-System"
DIST="$ROOT/dist"
NAME="claude-code-agency-client-delivery-system-v${VERSION}.zip"
ARCHIVE="$DIST/$NAME"
...
python3 "$ROOT/scripts/validate-agency-system.py" || die "validation failed"
ok "bundle valid"
...
# Same definitions as scripts/validate-agency-system.py. If they drift, the
...
scripts = [r for r in files if r.startswith("scripts/") and r.endswith(".py")]
templates = [r for r in files if r.startswith("templates/") and not r.startswith("templates/client-project/") and r != "templates/README.md"]
...
worksheets = [r for r in files if r.startswith("worksheets/") and r != "worksheets/README.md"]
examples = [r for r in files if r.startswith("examples/")]

lines = [
  "# Product manifest", "",
  "**Claude Code Agency & Client Delivery System** — version %s" % version, "",
...
print("  files=%d modules=%d docs=%d commands=%d scripts=%d templates=%d reports=%d checklists=%d worksheets=%d examples=%d words=%s"
      % (len(files), len(modules), len(docs), len(commands), len(scripts), len(templates), len(reports), len(checklists), len(worksheets), len(examples), format(words, ",")))
PY
ok "manifest written"

say "Re-validating with the manifest present"
python3 "$ROOT/scripts/validate-agency-system.py" || die "validation failed after manifest"
ok "still valid"
