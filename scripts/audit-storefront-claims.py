...
META_SUBJECT = (
    ...
    ("Shopify Toolkit", "claude-code-shopify-automation-admin-api-toolkit"),
)
...
SHOP_BUNDLE = os.path.join(REPO, "product", "Claude-Code-Shopify-Automation-Admin-API-Toolkit")
SHOP_SOURCES = ("product.shopify-toolkit.json",)
MIG_BUNDLE = os.path.join(REPO, "product", "Claude-Code-Website-Migration-Replatforming-System")
MIG_SOURCES = ("product.migration-system.json",)
AGENCY_BUNDLE = os.path.join(REPO, "product", "Claude-Code-Agency-Client-Delivery-System")
AGENCY_SOURCES = ("product.agency-system.json",)
...
BY_HANDLE = {
    ...
    "claude-code-shopify-automation-admin-api-toolkit": "shop",
    "claude-code-website-migration-replatforming-system": "migration",
    "claude-code-agency-client-delivery-system": "agency",
}
...
def measure(bundle):
    ...
    if bundle == MIG_BUNDLE:
        # templates/redirects/ holds one sample per platform; each is a template
        templates = sum(len([f for f in fn if f != "README.md"]) for dp, dn, fn in os.walk(os.path.join(bundle, "templates")))
    if bundle == AGENCY_BUNDLE:
        # the client-project skeleton is one template made of directories; the files beside it are the templates
        td = os.path.join(bundle, "templates")
        templates = len([f for f in os.listdir(td) if os.path.isfile(os.path.join(td, f)) and f != "README.md"])
    commands = 0
    ...
    # The Shopify toolkit keeps its commands in .claude/commands/, one file each.
    cdir2 = os.path.join(bundle, ".claude", "commands")
    if not commands and os.path.isdir(cdir2):
        commands = len([f for f in os.listdir(cdir2) if f.endswith(".md") and f != "COMMANDS.md"])
    sdir = os.path.join(bundle, "scripts")
    scripts = len([f for f in os.listdir(sdir) if f.endswith(".py") and not f.startswith("_")]) if os.path.isdir(sdir) else 0
    gdir = os.path.join(bundle, "graphql")
    graphql = len([f for f in os.listdir(gdir) if f.endswith(".graphql")]) if os.path.isdir(gdir) else 0
    workflows = 0
    for d in sorted(os.listdir(bundle)):
        if re.match(r"^(0[1-9]|1[0-2])-", d) and os.path.isdir(os.path.join(bundle, d)):
            workflows += len([f for f in os.listdir(os.path.join(bundle, d))
                              if f.endswith(".md")])
    # platform guides and report templates live in a numbered module (ops) or a top-level directory (migration, shopify)
    def md_count(*subs):
        for sub in subs:
            d = os.path.join(bundle, sub)
            if os.path.isdir(d):
                return len([f for f in os.listdir(d) if f.endswith(".md") and f != "README.md"])
        return 0
    guides = md_count("20-platform-guides", "17-platforms")
    reports = md_count("reports", "15-reporting")
    return {"files": files, "modules": len(mods), "words": words, "guides": guides, "reports": reports, "worksheets": count("worksheets"),
...
SHOP_TRUTH = measure(SHOP_BUNDLE)
MIG_TRUTH = measure(MIG_BUNDLE)
AGENCY_TRUTH = measure(AGENCY_BUNDLE)
...
for label, t in (("SEO toolkit", SEO_TRUTH), ("Conversion toolkit", CRO_TRUTH), ("Operations system", OPS_TRUTH), ("Shopify toolkit", SHOP_TRUTH)):
    if t:
...
def truth_for(source):
    ...
    if SHOP_TRUTH and source in SHOP_SOURCES:
        return SHOP_TRUTH, "Shopify toolkit"
    if MIG_TRUTH and source in MIG_SOURCES:
        return MIG_TRUTH, "Migration system"
    if AGENCY_TRUTH and source in AGENCY_SOURCES:
        return AGENCY_TRUTH, "Agency system"
...
CROSS = [
    ...
    ("Shopify Automation & Admin API Toolkit", lambda: (SHOP_TRUTH, "Shopify toolkit")),
    ("Shopify Automation Toolkit", lambda: (SHOP_TRUTH, "Shopify toolkit")),
    ("Website Migration & Replatforming System", lambda: (MIG_TRUTH, "Migration system")),
    ("Migration System", lambda: (MIG_TRUTH, "Migration system")),
    ("Agency & Client Delivery System", lambda: (AGENCY_TRUTH, "Agency system")),
    ("Agency System", lambda: (AGENCY_TRUTH, "Agency system")),
]
...
CHECKS = [
    ...
    (r"\b(\d+)\s+(?:validated\s+)?GraphQL documents\b", "graphql", None),
    (r"\b(\d+)\s+platform guides\b", "guides", None),
    (r"\b(\d+)\s+report templates\b", "reports", None),
    (r"\b(\d+)\s+worksheets\b", "worksheets", None),
    (r"\b(\d+)\s+report shapes\b", "reports", None),
]

TRUTHS.update({"launch": truth, "seo": SEO_TRUTH, "cro": CRO_TRUTH,
               "stack": STACK_TRUTH, "ops": OPS_TRUTH, "shop": SHOP_TRUTH, "migration": MIG_TRUTH, "agency": AGENCY_TRUTH})
...
TILE_DIR = {
    ...
    "product.shopify-toolkit.json": SHOP_BUNDLE,
    "product.migration-system.json": MIG_BUNDLE,
    "product.agency-system.json": AGENCY_BUNDLE,
}
for source, lab, meta in sorted(set(module_tiles)):
    ...
    if not dirs:
        ...
        continue
    if root == MIG_BUNDLE:
        # each migration module is its README plus supporting files; the README is the module
        actual = len(os.listdir(os.path.join(root, dirs[0])))
    elif root in (OPS_BUNDLE, SHOP_BUNDLE):
