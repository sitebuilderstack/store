...
PRODUCTS = {
    ...
    "shopify": {
        "bundle": "Claude-Code-Shopify-Automation-Admin-API-Toolkit",
        "title": ["Shopify Automation &", "Admin API Toolkit"],
        "out": "product-shopify-toolkit.png",
        "unit": "files",
        "hot": {"16 STORE AUDITING", "COMMANDS", "SCRIPTS"},
        "extras": [("COMMANDS", "{commands} commands"), ("SCRIPTS", "{scripts} scripts"),
                   ("GRAPHQL", "{graphql} documents")],
        "template_dir": "19-",
        "template_noun": "files",
        "validator": "validate-shopify-toolkit.py",
    },
    "migration": {
        "bundle": "Claude-Code-Website-Migration-Replatforming-System",
        "title": ["Website Migration &", "Replatforming System"],
        "out": "product-migration-system.png",
        "unit": "files",
        "hot": {"06 MIGRATION MAPPING", "12 REDIRECTS", "17 POST MIGRATION VALIDATION", "COMMANDS", "SCRIPTS"},
        "extras": [("COMMANDS", "{commands} commands"), ("SCRIPTS", "{scripts} scripts"),
                   ("CHECKLISTS", "{checklists} checklists"), ("EXAMPLE", "1 complete migration")],
        "template_dir": None,
        "template_noun": None,
        "validator": "validate-migration-system.py",
    },
    "agency": {
        "bundle": "Claude-Code-Agency-Client-Delivery-System",
        "title": ["Agency & Client", "Delivery System"],
        "out": "product-agency-system.png",
        "unit": "files",
        "hot": {"05 PROJECT SCOPING", "07 PROPOSALS", "17 LAUNCH", "COMMANDS"},
        "extras": [("COMMANDS", "{commands} commands"), ("TEMPLATES", "{templates} templates"),
                   ("CHECKLISTS", "{checklists} checklists")],
        "template_dir": None,
        "template_noun": None,
        "validator": "validate-agency-system.py",
    },
    "seo": {
...
AGENCY_CMDS = {"01": "/lead-qualification", "02": "/client-discovery", "03": "/client-intake", "04": "/client-website-audit", "05": "/client-scope",
               "06": "/project-estimate", "07": "/generate-proposal", "08": "/client-onboarding", "09": "/requirements-document", "10": "/project-plan",
               "11": "/setup-client-project", "12": "/delivery-workflow", "13": "/weekly-client-report", "14": "/change-request", "15": "/client-project-qa",
               "16": "/client-review-package", "17": "/client-launch-readiness", "18": "/client-handoff", "19": "/monthly-client-report", "20": "/retainer-opportunity",
               "21": "/client-offboarding"}
...
def module_counts():
    ...
    for name in sorted(os.listdir(BUNDLE)):
        ...
        # The migration system's modules are a README each plus supporting files; the README is the module.
        n = len([f for f in os.listdir(path)
                 if (KEY in ("ops", "shopify", "migration", "agency") or f.endswith(".md")) and (f != "README.md" or KEY in ("migration", "agency"))])
        pretty = name.split("-", 1)[1].replace("-", " ")
        noun = P["unit"]
        if KEY == "agency":
            # each module is one document; the useful fact on a tile is the command it drives
            out.append(("%s %s" % (name[:2], pretty.upper()), AGENCY_CMDS.get(name[:2], "")))
            continue
...
def flat_counts():
    ...
    if KEY == "agency":
        # templates are md, csv, yaml, json and dotfiles; the client-project skeleton is one template, counted by the validator's rule
        c["templates"] = len([f for f in os.listdir(os.path.join(BUNDLE, "templates")) if os.path.isfile(os.path.join(BUNDLE, "templates", f)) and f != "README.md"])
    ...
    c["scripts"] = len([f for f in os.listdir(sdir) if f.endswith(".py") and not f.startswith("_")]) if os.path.isdir(sdir) else 0
    gdir = os.path.join(BUNDLE, "graphql")
    c["graphql"] = len([f for f in os.listdir(gdir) if f.endswith(".graphql")]) if os.path.isdir(gdir) else 0
    cdir2 = os.path.join(BUNDLE, ".claude", "commands")
    if os.path.isdir(cdir2):
        c["commands"] = len([f for f in os.listdir(cdir2) if f.endswith(".md") and f != "COMMANDS.md"])
...
if KEY == "cro":
    ...
    strap = "AUTOMATE SHOPIFY  ·  Products • Collections • SEO • Metafields • Inventory • Bulk"
elif KEY == "migration":
    strap = "MIGRATE  ·  Discover • Map • Move • Validate"
elif KEY == "agency":
    strap = "DELIVER  ·  Discover • Scope • Build • Launch • Retain"
else:
...
for i, (label, meta) in enumerate(MODULES):
    ...
    while d.textlength(label, font=font(MONO_B, size)) > CW - 40 and size > 12:
        size -= 1
    d.text((cx + 20, cy + min(26, CH * 0.24)), label, font=font(MONO_B, size), fill=TEXT)
    ...
    # With six rows the tile is shorter than the 116px the layout was drawn for;
    # keep the meta line inside the tile instead of under its bottom edge.
    d.text((cx + 20, cy + min(70, CH - 36)), meta, font=font(MONO, 19), fill=ACCENT if hot else DIM)
...
if KEY == "ops":
    ...
    foot = ("%d modules  ·  %d commands  ·  %d scripts  ·  Admin GraphQL API 2026-07  ·  not affiliated with Shopify"
            % (counts["modules"], flat["commands"], flat["scripts"]))
elif KEY == "migration":
    foot = ("%d modules  ·  %d commands  ·  %d scripts  ·  Python 3 standard library only  ·  7 platform guides"
            % (counts["modules"], flat["commands"], flat["scripts"]))
elif KEY == "agency":
    foot = ("%d modules  ·  %d commands  ·  %d templates  ·  %d checklists  ·  one complete example engagement"
            % (counts["modules"], flat["commands"], flat["templates"], flat["checklists"]))
