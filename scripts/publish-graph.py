...
def article_entries(graph, arts):
    ...
    for h, a in graph["articles"].items():
        ...
        # The "Try this on your project" module rendered inside the body at
        # the author's <div data-sbs-try-slot></div>. The id must exist in
        # graph.modules and in snippets/sbs-try.liquid (validate-graph and
        # test-engagement.js check both).
        if a.get("module"):
            if a["module"] not in graph.get("modules", {}):
                raise SystemExit("%s: unknown module %s" % (h, a["module"]))
            add("try", a["module"])
