...
# ---------------------------------------------------------- diagram 29
def d29_maintenance_cadence():
    """Daily / weekly / monthly columns; (host) tags on platform-run items."""
    ACC = (94, 234, 212)
    cols = [
        ("DAILY", "2 minutes", [("Homepage + key page 200", False), ("No incident overnight", False)]),
        ("WEEKLY", "15-30 minutes", [("Marked form test", False), ("Nav + footer destinations", False),
                                     ("Broken links / 404 log", False), ("Certificate > 14 days", True),
                                     ("Security headers", True), ("Analytics real-time", False)]),
        ("MONTHLY", "1-2 hours", [("Dependencies, staging first", True), ("Configuration vs baseline", False),
                                  ("Restore a backup, open it", False), ("Rollback rehearsed", False),
                                  ("Performance vs last month", False), ("Report from evidence", False)]),
    ]
    rows = max(len(c[2]) for c in cols)
    h = 132 + 52 + rows * 40 + 84
    img, d = canvas(h)
    head(d, "Daily, weekly, monthly: sorted by how fast each thing fails",
         "Items marked host are run by a hosted platform; you verify what remains.", ACC)
    cw = (W - 80 - 2 * 16) // 3
    for i, (name, time_, items) in enumerate(cols):
        x = 40 + i * (cw + 16)
        d.rounded_rectangle([x, 132, x + cw, h - 70], 10, fill=SURFACE2, outline=BORDER, width=1)
        d.text((x + 18, 148), name, font=f(MONO_B, 16), fill=ACC)
        tw = d.textlength(time_, font=f(SANS, 13))
        d.text((x + cw - 18 - tw, 151), time_, font=f(SANS, 13), fill=DIM)
        d.line([(x + 18, 178), (x + cw - 18, 178)], fill=BORDER, width=1)
        y = 192
        for label, host in items:
            d.ellipse([x + 18, y + 6, x + 28, y + 16], outline=ACC, width=2)
            d.text((x + 38, y), label, font=f(SANS, 14), fill=TEXT)
            if host:
                lw = d.textlength(label, font=f(SANS, 14))
                d.rounded_rectangle([x + 44 + lw, y + 1, x + 44 + lw + 44, y + 19], 5, fill=mix(ACC, BG, .18), outline=ACC, width=1)
                d.text((x + 50 + lw, y + 3), "host", font=f(MONO, 11), fill=ACC)
            y += 40
    foot(d, h, "The weekly form test is the most valuable line and the one most often skipped.")
    save(img, "diagram-maintenance-cadence.png")


# ---------------------------------------------------------- diagram 30
def d30_redirect_outcomes():
    """Five redirect outcomes; only the first and last pass."""
    ACC = (134, 239, 172)
    RED = (248, 113, 113)
    rows = [
        ("PASS", "/physio  301 →  /services/physiotherapy  200", "One hop, mapped destination, identity matches"),
        ("FAIL", "/old-services  301 →  /services-2  301 →  /services/  200", "A chain: the old redirect was kept instead of merged"),
        ("FAIL", "/loop-a  301 →  /loop-b  301 →  /loop-a  …", "A loop: a rule for the old host applied on the new one"),
        ("FAIL", "/team  301 →  /  200", "Right status, wrong place: the homepage dump, a soft 404"),
        ("PASS", "/christmas-2019  410", "Honest removal: no equivalent page, so say so"),
    ]
    h = 132 + len(rows) * 74 + 78
    img, d = canvas(h)
    head(d, "Three of the five return the right status code",
         "A redirect check compares destinations and hop counts, not just 301s.", ACC)
    y = 142
    for verdict, chain, why in rows:
        ok = verdict == "PASS"
        col = ACC if ok else RED
        d.rounded_rectangle([40, y, W - 40, y + 60], 8, fill=mix(col, BG, .08), outline=col, width=2)
        d.text((60, y + 21), verdict, font=f(MONO_B, 15), fill=col)
        d.text((136, y + 12), chain, font=f(MONO, 14), fill=TEXT)
        d.text((136, y + 36), why, font=f(SANS, 13), fill=MUTED)
        y += 74
    foot(d, h, "Google: many old URLs to one irrelevant destination may be treated as a soft 404.")
    save(img, "diagram-redirect-outcomes.png")


# ---------------------------------------------------------- diagram 31
def d31_test_loop():
    """Write → pass → break → fail → restore → pass; the middle earns the trust."""
    ACC = (138, 166, 255)
    steps = [("1", "Write", "Assert what the visitor sees", False),
             ("2", "Run", "Green", False),
             ("3", "Break", "Disable the check on purpose", True),
             ("4", "Fail", "Received: \"Thanks! We will email you\"", True),
             ("5", "Restore", "Green again — now it is a test", False)]
    h = 132 + 150 + 70
    img, d = canvas(h)
    head(d, "A test earns trust in the middle of the loop",
         "A suite that has never been seen failing is a suite that may pass for the wrong reason.", ACC)
    bw = (W - 80 - 4 * 14) // 5
    y = 150
    for i, (n, what, why, hot) in enumerate(steps):
        x = 40 + i * (bw + 14)
        d.rounded_rectangle([x, y, x + bw, y + 110], 10,
                            fill=mix(ACC, BG, .16) if hot else SURFACE2,
                            outline=ACC if hot else BORDER, width=3 if hot else 1)
        d.ellipse([x + 14, y + 14, x + 42, y + 42], fill=mix(ACC, BG, .22), outline=ACC, width=2)
        d.text((x + 23, y + 19), n, font=f(MONO_B, 16), fill=ACC)
        d.text((x + 52, y + 20), what, font=f(SANS_B, 16), fill=TEXT)
        # wrap the why line inside the box
        words, line, yy = why.split(), "", y + 56
        for w_ in words + [None]:
            trial = (line + " " + w_).strip() if w_ else line
            if w_ is None or d.textlength(trial, font=f(SANS, 12)) > bw - 28:
                d.text((x + 14, yy), line, font=f(SANS, 12), fill=ACC if hot else MUTED)
                yy += 17; line = w_ or ""
            else:
                line = trial
        if i < len(steps) - 1:
            d.line([(x + bw, y + 55), (x + bw + 14, y + 55)], fill=ACC if hot else BORDER, width=2)
    foot(d, h, "Step 4 is where a loose assertion like /email/i is caught: it matched the success message.")
    save(img, "diagram-test-loop.png")
...
if __name__ == "__main__":
    ...
    d29_maintenance_cadence(); d30_redirect_outcomes(); d31_test_loop()
