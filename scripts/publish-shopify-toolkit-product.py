#!/usr/bin/env python3
"""Create or update the Shopify Automation & Admin API Toolkit in Shopify.

Every quantity on the listing is read from the built bundle rather than typed:
scripts/validate-shopify-toolkit.py counts files, modules, commands and scripts, and
...
_spec = _il.spec_from_file_location("vo", os.path.join(HERE, "validate-shopify-toolkit.py"))
_vo = _il.module_from_spec(_spec)
_spec.loader.exec_module(_vo)

ROOT = os.path.dirname(HERE)
HANDLE = "claude-code-shopify-automation-admin-api-toolkit"
TITLE = "Claude Code Shopify Automation & Admin API Toolkit"
PRICE = "39.99"
FORBIDDEN_PRICES = ("$39.00", "$39.95", "$49", "$59", "$69", "$79", "$89")
SET_PRICE = "--set-price" in sys.argv

SKU = "SBS-CCSHOP-V1"
SEO_TITLE = "Claude Code Shopify Automation & Admin API Toolkit | SiteBuilderStack"
SEO_DESC = ("Automate Shopify products, SEO, collections, metafields, inventory, redirects and bulk "
            "operations with Claude Code and Shopify's Admin GraphQL API.")
...
def audit_description(body, counts):
    ...
    for name, n in (("files", counts["files"]), ("modules", counts["modules"]),
                    ("commands", counts["commands"]), ("scripts", counts["scripts"]), ("graphql", counts["graphql"])):
    ...
    for m in re.finditer(r"\$(\d+(?:\.\d+)?)", body):
        if m.group(0) not in ("$%s" % PRICE, "$19.99", "$39.99"):
...
def main():
    ...
    body = io.open(os.path.join(ROOT, "content", "products", "shopify-toolkit.html"), encoding="utf-8").read()
    print("counted from the bundle: %d files, %d modules, %d commands, %d scripts, %d GraphQL documents, "
          "%d checklists, %d report templates, %s words"
          % (counts["files"], counts["modules"], counts["commands"], counts["scripts"], counts["graphql"],
    ...
    payload = {
        ...
        "status": "ACTIVE",
        "tags": ["shopify", "automation", "admin-api", "graphql", "claude-code", "digital"],
        "templateSuffix": "shopify-toolkit",
    ...
    print("  template suffix  shopify-toolkit")
    return 0
