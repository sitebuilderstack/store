...
(async () => {
  ...
  const shopify = await answer(page, [2, 5, 0, 1, 5]);
  check('live + Shopify catalogue at scale -> Shopify toolkit',
        shopify.key === 'shopify' && shopify.count === 1, `${shopify.key} (${shopify.title})`);

  const migrate = await answer(page, [5, 6, 0, 1, 6]);
  check('about to move + moving problem -> Migration system',
        migrate.key === 'migrate' && migrate.count === 1, `${migrate.key} (${migrate.title})`);

  const deliver = await answer(page, [6, 7, 2, 1, 7]);
  check('builds for clients + running client projects -> Agency system',
        deliver.key === 'deliver' && deliver.count === 1, `${deliver.key} (${deliver.title})`);
  ...
  check('exactly one card is ever visible',
        [build, rank, convert, operate, shopify, migrate, deliver, all].every(r => r.count === 1));
  ...
  check('no-JS: the eight products remain as a readable list',
        fb.fallbackPainted && fb.products === 8, `${fb.products} products`);
  check('no-JS: the unusable form is not painted', !fb.formPainted);
  check('no-JS: the lifecycle stages still render', fb.stages === 8, String(fb.stages));
  await noJs.close();
