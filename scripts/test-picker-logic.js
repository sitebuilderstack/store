...
const t = (o) => Object.assign({ build: 0, rank: 0, convert: 0, operate: 0, shopify: 0, migrate: 0, deliver: 0, all: 0 }, o);

check('the key list is the eight products, bundle last', PICK_KEYS.join(',') === 'build,rank,convert,operate,shopify,migrate,deliver,all', PICK_KEYS.join(','));
...
check('Shopify-catalogue answers return the Shopify toolkit',
      pickWinner(t({ shopify: 7, rank: 1, convert: 1, operate: 1 })).key === 'shopify');
check('a live Shopify store with a ranking problem still returns the SEO toolkit',
      pickWinner(t({ rank: 8, shopify: 4, operate: 1, convert: 1 })).key === 'rank');
check('migration answers return the Migration system',
      pickWinner(t({ migrate: 10, rank: 1, convert: 1, operate: 3 })).key === 'migrate');
check('a live site with a ranking problem and a move ahead still returns the SEO toolkit when ranking dominates',
      pickWinner(t({ rank: 8, migrate: 3, operate: 1, convert: 1 })).key === 'rank');
check('client-delivery answers return the Agency system',
      pickWinner(t({ deliver: 12, all: 2, operate: 1 })).key === 'deliver');
check('an agency that owns the lifecycle for several sites still gets the bundle when the lifecycle answers dominate',
      pickWinner(t({ all: 15, deliver: 3 })).key === 'all');
...
check('the bundle loses a tie with every single product',
      ['build', 'rank', 'convert', 'operate', 'shopify', 'migrate', 'deliver'].every(k => pickWinner(t({ [k]: 7, all: 7 })).key === k));
