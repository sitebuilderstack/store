#!/usr/bin/env python3
"""Create or update the Agency & Client Delivery System in Shopify.

Every quantity on the listing is read from the built bundle rather than typed:
scripts/validate-agency-system.py counts files, modules, commands and scripts, and
...
Two listing-specific assertions:
  ...
  * The price is exactly 39.99 and the description states no other price for
    ...
    neighbour ($39, $39.00, $39.95, $49, $59, $79, $99, $129); a typo here
    would be the most visible possible mistake. The comparison table may quote
    the other six products' current prices ($19.99, $29.99, $39.99).
  * No predicted percentage improvement, no struck-through price, no urgency.

Digital product settings match the other six: no shipping, inventory not
...
Usage: publish-agency-system-product.py [--dry-run] [--check-only] [--set-price]
...
_spec = _il.spec_from_file_location("vo", os.path.join(HERE, "validate-agency-system.py"))
_vo = _il.module_from_spec(_spec)
_spec.loader.exec_module(_vo)

ROOT = os.path.dirname(HERE)
HANDLE = "claude-code-agency-client-delivery-system"
TITLE = "Claude Code Agency & Client Delivery System"
PRICE = "39.99"
FORBIDDEN_PRICES = ("$39.00", "$39.95", "$49", "$59", "$79", "$99", "$129")
SET_PRICE = "--set-price" in sys.argv

SKU = "SBS-CCACDS-V1"
SEO_TITLE = "Claude Code Agency & Client Delivery System | SiteBuilderStack"
SEO_DESC = ("Run professional website client projects with Claude Code. Includes discovery, audits, proposals, "
            "scoping, onboarding, QA, launch, handoff, reporting, and retainers.")
...
def audit_description(body, counts):
    problems = []
    for name, n in (("files", counts["files"]), ("modules", counts["modules"]), ("commands", counts["commands"]), ("scripts", counts["scripts"]),
                    ("templates", counts["templates"]), ("checklists", counts["checklists"]), ("reports", counts["reports"]), ("worksheets", counts["worksheets"])):
    ...
    for m in re.finditer(r"\$(\d+(?:\.\d+)?)", body):
        if m.group(0) not in ("$%s" % PRICE, "$19.99", "$29.99"):  # the comparison table: the other six products' prices
    ...
    # "$39", "$29" and "$19" may only appear as the start of $39.99 / $29.99 / $19.99
    for m in re.finditer(r"\$(19|29|39)(?!\.99)\b", body):
        problems.append("description contains a forbidden price: %s" % m.group(0))
    # $39.99 is also the Operations system's and the Shopify toolkit's price, so it appears in the comparison table too
...
def main():
    ...
    body = io.open(os.path.join(ROOT, "content", "products", "agency-system.html"), encoding="utf-8").read()
    print("counted from the bundle: %d files, %d modules, %d commands, %d scripts, %d templates, "
          "%d checklists, %d report templates, %d worksheets, %s words"
          % (counts["files"], counts["modules"], counts["commands"], counts["scripts"], counts["templates"],
             counts["checklists"], counts["reports"], counts["worksheets"], format(counts["words"], ",")))
    ...
    payload = {
        ...
        "status": "ACTIVE",
        "tags": ["agency", "freelance", "client-delivery", "proposals", "project-management", "claude-code", "digital"],
        "templateSuffix": "agency-system",
    ...
    print("  template suffix  agency-system")
    return 0
