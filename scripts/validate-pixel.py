...
must send only the event name, the label, the path, a coarse timestamp and,
for engagement events, an identifier.
...
# "m" (added 2026-09-17): the engagement events' public identifier — a
# module, lab, challenge or product handle, or a coarse kind — never free text.
# "r" (artifact attribution id, v_ + hex), "o" (order id for purchase
# dedupe) and "v" (line value) were added 2026-09-18 for the navigator funnel.
ALLOWED_KEYS = {"e", "l", "p", "t", "m", "r", "o", "v"}
...
def theme_events(root):
    """Every event name the storefront actually publishes.

    This is the list that matters: the other three are descriptions of it. They
    agreed with each other for weeks while all three omitted eight events the
    theme was firing, because nothing compared any of them against the theme.

    Four emission shapes: a literal track() call, a data-sbs-track attribute in
    Liquid or in a template's JSON, an entry in one of sbs.js's event tables,
    and a name carried in a config object as `event: 'x'`.
    """
    theme = os.path.join(root, "theme", "dev")
    out = set()
    js_path = os.path.join(theme, "assets", "sbs.js")
    if os.path.exists(js_path):
        with open(js_path, encoding="utf-8") as fh:
            js = fh.read()
        out |= set(re.findall(r"track\('([a-z_]+)'", js))
        out |= set(re.findall(r"data-sbs-track', '([a-z_]+)'", js))
        out |= set(re.findall(r"event: '([a-z_]+)'", js))
        b = re.search(r"ENGAGE_EVENTS = \{(.*?)\n  \};", js, re.S)
        if b:
            out |= set(re.findall(r"([a-z_]+):", b.group(1)))
        a = re.search(r"ARRIVAL_EVENT = '([a-z_]+)'", js)
        if a:
            out.add(a.group(1))
    for dirpath, _dirnames, filenames in os.walk(theme):
        for fn in filenames:
            if not fn.endswith((".liquid", ".json")):
                continue
            with open(os.path.join(dirpath, fn), encoding="utf-8", errors="replace") as fh:
                t = fh.read()
            # Only whole literal values. A Liquid-interpolated name like
            # data-sbs-track="lead_segment_{{ v }}" is not an event name; a
            # looser pattern captures the prefix and reports a phantom.
            out |= set(re.findall(r'data-sbs-track="([a-z_]+)"', t))
            out |= set(re.findall(r'data-sbs-track=\\"([a-z_]+)\\"', t))
    return out

def check(pixel_src, doc_src, worker_src=None, theme=None):
    """Findings. Empty list means the lists agree with each other AND the theme."""
    bad = []
    ...
    if worker_src is not None:
        ...
        wh = set(re.findall(r"'([a-z_]+)'", (re.search(r"const WEBHOOK_EVENTS = new Set\(\[(.*?)\]\);", worker_src, re.S) or [None, ""])[1]))
        for e in sorted(we - pe - wh):
            bad.append("accepted by the collector but never sent: %s" % e)
        # A documented webhook-borne event is collected without the pixel.
        bad = [b for b in bad if not any(b.endswith(": " + e) and "not subscribed" in b for e in wh)]
    if theme:
        te = theme_events(theme)
        pe_now = pixel_events(pixel_src)
        # Published by the storefront but not subscribed: the event fires into
        # nothing. This is the failure the other checks cannot see.
        for e in sorted(te - pe_now):
            bad.append("published by the theme but not subscribed by the pixel: %s" % e)
...
def main():
    ...
    bad = check(src, doc, wsrc, theme=ROOT)
